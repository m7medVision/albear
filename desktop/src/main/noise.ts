...
// (PRD 12.4): Noise_XX_25519_ChaChaPoly_SHA256 for pairing/CLI mode and
// Noise_XXpsk3_25519_ChaChaPoly_SHA256 for paired sessions, initiator side.
//
// Copied verbatim from extension/src/noise/noise.ts (keep the two in sync).
// Runs in the Electron MAIN process: Node's WebCrypto globals provide
// crypto.getRandomValues and the @noble primitives work as-is.
// Interoperability with the Go daemon (flynn/noise) is pinned by the
// cross-language vectors in ./testdata/vectors.json (see __tests__).
...
export class CipherState {
  ...
  private maybeRekey(): void {
    if (this.n !== 0n && this.n % CipherState.REKEY_INTERVAL === 0n)
      this.rekey();
  }
  ...
  rekey(): void {
    ...
    const ct = chacha20poly1305(this.k, nonce, new Uint8Array(0)).encrypt(
      new Uint8Array(32),
    );
...
class SymmetricState {
  ...
  constructor(protocolName: string) {
    ...
    this.h =
      name.length <= 32
        ? concat(name, new Uint8Array(32 - name.length))
        : sha256(name);
  ...
  mixKeyAndHash(input: Uint8Array): void {
    const [ck, tempH, tempK] = hkdf(this.ck, input, 3) as [
      Uint8Array,
      Uint8Array,
      Uint8Array,
    ];
  ...
  split(): [CipherState, CipherState] {
    const [k1, k2] = hkdf(this.ck, new Uint8Array(0), 2) as [
      Uint8Array,
      Uint8Array,
    ];
